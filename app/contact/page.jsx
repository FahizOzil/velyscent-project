...
const INFO = [
  {
    icon: (
      <svg width="22" height="22" fill="none" stroke="#C4914F" strokeWidth="1.6" viewBox="0 0 24 24">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "16px", marginBottom: "48px" }}>
        {INFO.map((item) => (
          <div key={item.label} style={{
            ...
            onMouseEnter={(e) => e.currentTarget.style.borderColor = "rgba(196,145,79,0.3)"}
            onMouseLeave={(e) => e.currentTarget.style.borderColor = "rgba(196,145,79,0.12)"}
          >
            {/* Icon */}
            <div style={{ width: "44px", height: "44px", borderRadius: "50%", background: "rgba(196,145,79,0.08)", border: "1px solid rgba(196,145,79,0.15)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: "16px" }}>
              {item.icon}
            </div>
            ...
            {/* Value */}
            {item.href ? (
              <a href={item.href} target={item.href.startsWith("http") ? "_blank" : undefined} rel="noreferrer" style={{ fontFamily: "'Jost',sans-serif", fontSize: "16px", fontWeight: 500, color: "#FFFFFF", textDecoration: "none", display: "block", marginBottom: "6px", transition: "color 0.2s" }}
