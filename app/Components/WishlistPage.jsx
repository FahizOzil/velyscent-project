...
function WishlistCard({ item }) {
  ...
  const handleAddToCart = () => {
    addToCart({
      ...
      variant: item.volume, image: item.image || "",
      volume: item.volume, qty: 1,
    });
    setAdded(true);
  ...
  return (
    <div style={{
      ...
      onMouseEnter={(e) => { e.currentTarget.style.border = "1px solid rgba(196,145,79,0.3)"; e.currentTarget.style.transform = "translateY(-3px)"; e.currentTarget.style.boxShadow = "0 12px 32px rgba(0,0,0,0.4)"; }}
    ...
    >
      ...
      <Link href={`/shop/${item.slug}`} style={{ textDecoration: "none" }}>
        <div style={{
          ...
          {item.image ? (
            ...
            <svg width="32" height="32" fill="none" stroke="rgba(196,145,79,0.3)" strokeWidth="1.2" viewBox="0 0 24 24">
      ...
      <div style={{ padding: "16px 16px 20px", display: "flex", flexDirection: "column", flexGrow: 1 }}>
        <Link href={`/shop/${item.slug}`} style={{ textDecoration: "none" }}>
          <h3 style={{ fontFamily: "'Jost',sans-serif", fontSize: "14px", fontWeight: 400, color: "#FFFFFF", marginBottom: "6px", transition: "color 0.2s" }}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "16px" }}>
          <span style={{ fontFamily: "'Jost',sans-serif", fontSize: "14px", fontWeight: 500, color: "#C4914F" }}>
        ...
        <div style={{ display: "flex", gap: "8px", marginTop: "auto" }}>
          <button onClick={handleAddToCart} style={{
            ...
            border: "1px solid #C4914F", borderRadius: "5px", color: "#C4914F",
            fontFamily: "'Jost',sans-serif", fontSize: "11px",
            fontWeight: 600, letterSpacing: "0.06em",
            textTransform: "uppercase", cursor: "pointer", transition: "all 0.2s",
          }}
            onMouseEnter={(e) => e.currentTarget.style.background = "rgba(196,145,79,0.15)"}
            onMouseLeave={(e) => { if (!added && !inCart) e.currentTarget.style.background = "transparent"; }}
          >
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 520px)  { .wishlist-grid { grid-template-columns: 1fr !important; } }
      `}</style>
      ...
      <section style={{ width: "100%", background: "#0A0806", minHeight: "100vh", padding: "100px 0 96px" }}>
        <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "0 40px" }}>
          ...
          {/* Header */}
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "40px", flexWrap: "wrap", gap: "16px", animation: "fadeUp 0.8s ease forwards" }}>
            <h1 style={{ fontFamily: "'Playfair Display',Georgia,serif", fontSize: "clamp(24px,3vw,36px)", fontWeight: 600, color: "#C4914F", letterSpacing: "0.01em" }}>
              My Wishlist{" "}
              {items.length > 0 && (
            ...
            {items.length > 0 && (
              <button onClick={clearWishlist} style={{
                ...
                fontSize: "12px", color: "rgba(255,100,100,0.5)", cursor: "pointer",
          ...
          {items.length === 0 && (
            <div style={{ textAlign: "center", padding: "80px 0", animation: "fadeUp 0.8s ease forwards" }}>
              ...
              <p style={{ fontFamily: "'Playfair Display',serif", fontSize: "22px", color: "rgba(255,255,255,0.5)", marginBottom: "12px" }}>Your wishlist is empty</p>
              <p style={{ fontFamily: "'Jost',sans-serif", fontSize: "14px", fontWeight: 300, color: "rgba(255,255,255,0.3)", marginBottom: "32px" }}>Save your favourite fragrances here</p>
          ...
          {items.length > 0 && (
            <>
              <div className="wishlist-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "20px", marginBottom: "48px" }}>
                {items.map((item) => <WishlistCard key={item.slug} item={item} />)}
              </div>

              {/* Bottom CTA */}
              <div style={{ display: "flex", justifyContent: "center", gap: "16px", paddingTop: "32px", borderTop: "1px solid rgba(255,255,255,0.06)", flexWrap: "wrap" }}>
                <Link href="/shop" style={{
                  padding: "13px 32px", background: "transparent",
                  border: "1px solid rgba(255,255,255,0.15)", borderRadius: "6px",
                  color: "rgba(255,255,255,0.65)", fontFamily: "'Jost',sans-serif",
